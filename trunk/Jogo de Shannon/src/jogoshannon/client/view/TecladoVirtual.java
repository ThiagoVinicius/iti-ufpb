...
public class TecladoVirtual extends Composite 
implements HasValueChangeHandlers<Character>, ValueChangeHandler<Character> {
	...
	private class Teclado {
		...
		private HashMap<Character, BotaoTeclado> teclas = 
						new HashMap<Character, BotaoTeclado>();
	...
	private Panel montarTeclado(Teclado teclado) {
		...
		for (int i = 0; i < BOTOES_TECLAS.length; ++i) {
			...
			for (int j = 0; j < BOTOES_TECLAS[i].length; ++j) {
				BotaoTeclado novo = new BotaoTeclado(
						NOMES_TECLAS[i][j], BOTOES_TECLAS[i][j], teclado);
				hPanel.add(novo);
			}
		}
